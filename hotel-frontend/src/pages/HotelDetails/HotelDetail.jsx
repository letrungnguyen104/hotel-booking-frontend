...
const RoomSelectionPanel = ({ roomType, hotelId, onAddToCart }) => {
  ...
  if (loading) return <div className="panel-loading"><Spin /></div>;
  ...
  return (
    <div className="room-selection-panel">
          ...
          <Space direction="vertical">
            {services.map(s => (
              <Checkbox key={s.id} value={s.id}>
                {s.name} - <span className="service-price">{s.price.toLocaleString()} VND</span>
              </Checkbox>
            ))}
          </Space>
        </Checkbox.Group>
      ...
      <Button type="primary" onClick={handleAddToCartClick} style={{ marginTop: '16px' }}>
        Add to Booking
      </Button>
    </div>
  );
...
const BookingSummary = ({ cart, onRemoveItem, onCheckout }) => {
  ...
  const total = cart.reduce((acc, item) => {
    ...
    const servicesTotal = item.selectedServices.reduce((sAcc, s) => sAcc + s.price, 0);
  ...
  return (
    <div className="booking-summary-bar">
        ...
        <ul>
          {cart.map((item, index) => (
            <li key={item.cartItemId}>
              <div>
                <span>
                  1 x {item.roomType.name}
                </span>
                <Button type="link" danger onClick={() => onRemoveItem(index)}>Remove</Button>
                {item.selectedServices.length > 0 && (
                  <div className="summary-services">
                    Services: {item.selectedServices.map(s => s.name).join(', ')}
                  </div>
                )}
              </div>
            </li>
      ...
      <div className="summary-action">
        <span className="total-price">Total: {total.toLocaleString()} VND</span>
...
const HotelDetail = () => {
  ...
  useEffect(() => {
    ...
    if (hotelInCart && hotelInCart.id !== hotelId) {
      ...
      toast.info("Your cart has been cleared as you are viewing a different hotel.");
    }
  ...
  if (loading) { return <div className="hotel-detail-loading"><Spin size="large" /></div>; }
  if (error || !hotel) { return <div className="hotel-detail-error"><h1>Hotel not found</h1><p>{error}</p></div>; }
    ...
    <>
      <div className="hotel-detail">
            ...
            <Row gutter={[8, 8]} className="gallery-row">
              <Col xs={24} md={12}>
                <div className="main-image-wrapper">
                  <Image
                    src={hotel.images?.[0] || "https://via.placeholder.com/800x600?text=No+Image"}
                    alt={`Hotel gallery 1`}
                  />
                </div>
              </Col>
              <Col xs={24} md={12}>
                <Row gutter={[8, 8]} className="sub-images-row">
                  ...
                  {hotel.images?.slice(1, 5).length < 4 && Array.from({ length: 4 - hotel.images?.slice(1, 5).length }).map((_, index) => (
                    <Col xs={12} key={`placeholder-${index}`}>
                      <div className="thumb-image-wrapper placeholder-image">
                        <Image src="https://via.placeholder.com/400x300?text=No+Image" alt="Placeholder" />
                      </div>
                    </Col>
                  ))}
                </Row>
        ...
        <div className="rooms-section">
          <h3>Available Room Types {checkIn && `from ${dayjs(checkIn).format('DD/MM/YYYY')} to ${dayjs(checkOut).format('DD/MM/YYYY')}`}</h3>

          {roomTypes.length > 0 ? roomTypes.map(rt => {
            const roomsOfThisTypeInCart = bookingCart.filter(item => item.roomType.id === rt.id).length;
            ...
            return (
              <React.Fragment key={rt.id}>
                <Card className={`room-card-customer ${!isAvailable ? 'disabled' : ''}`}>
                  <Row gutter={[24, 24]} align="middle">
                    <Col xs={24} md={8}>
                      <Image src={rt.images?.[0] || "https://via.placeholder.com/400x300?text=No+Image"} alt={rt.name} className="room-image" />
                    </Col>
                    <Col xs={24} md={10}>
                      <h4>{rt.name}</h4>
                      <p>{rt.description}</p>
                    ...
                    <Col xs={24} md={6} className="booking-details">
                      <div className="price-info">
                        {hasDiscount && (<span className="old-price">{rt.oldPrice?.toLocaleString()} VND</span>)}
                        <span className="price-value">{(rt.newPrice || rt.pricePerNight)?.toLocaleString()} VND</span>
                      ...
                      <Button type="primary" className="booking-button" disabled={!isAvailable} onClick={() => handleToggleSelect(rt.id)}>
                        {openRoomTypeId === rt.id ? 'Close' : 'Select Room'}
                      </Button>
                    </Col>
                  </Row>
                </Card>
